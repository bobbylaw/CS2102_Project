...
*/
CREATE OR REPLACE PROCEDURE add_course(IN course_title TEXT, IN course_description TEXT, IN course_area TEXT, IN course_duration INTERVAL)
AS $$
BEGIN
...
BEGIN
    ...
    RETURN query (
        ...
        FROM Rooms 
        EXCEPT -- select all the rooms except which ones are occupied at any time within the interval
        SELECT rid AS room_id
        FROM Sessions s
        WHERE input_session_date = session_date 
        AND ( --all the four possible situations that the session time has overlap with the input interval
            (input_start_hour < extract(hour from start_time) 
            AND (input_start_hour + input_duration) > extract(hour from start_time) 
            AND (input_start_hour + input_duration) < extract(hour from end_time)) 
            OR 
            (input_start_hour >= extract(hour from start_time) 
            AND input_start_hour < extract(hour from end_time) 
            AND (input_start_hour + input_duration) > extract(hour from end_time))
            OR
            (input_start_hour >= extract(hour from start_time) 
            AND input_start_hour <= extract(hour from end_time) 
...
1. I have to find the availability information of all the rooms
    Implementation: It is done in the cursor. It looks through every room. (rid asc order)
2. For each day from input_start_date and input_end_date, I need to check each room avaliable HOURS. 
        Since there is no sessions before 9:00, between 12:00 to 14:00, and after 18:00, I only checked the availability for reasonable hours.
   Implementation: There are 2 loops. Outer loop is loop through the cursor(all the rooms). Outer loop is loop through the date and time.
        For the inner loop, the IF in the first if-else clause is to transition to the next day and add an entry into the table if there is avaliable hours during that day.
        The ElSIF is to transition from 12:00 to 14:00 because that's the break time.
        The else in the first if-else clause is to check whether there is any hours that the room is free on that day. 
        The select query help me check if the room is avaliable for that HOUR.
        So if the room is avaliable on that hour, i append to the avaliable_hours array.
        The loop only ends after start_date is equal to end_date + 1 (the information on end_date is included in the answer). 
        Then I proceed do the same for the next room.
*/
CREATE OR REPLACE FUNCTION get_available_rooms(IN input_start_date DATE, IN input_end_date DATE)
RETURNS TABLE (room_id INTEGER, room_capacity INTEGER, day DATE, available_hours TIME[]) --or month INTEGER, day INTEGER?
AS $$
DECLARE
    curs CURSOR FOR (
        SELECT rid, seating_capacity 
        FROM Rooms
        ORDER BY rid ASC
    );
    r RECORD;
...
BEGIN
    ...
    LOOP
        ...
        EXIT WHEN NOT FOUND;
        LOOP
            EXIT WHEN start_date_helper = input_end_date + 1; --include the end date
...
*/
CREATE OR REPLACE PROCEDURE add_course_offerings (IN course_title TEXT, IN course_fees NUMERIC(12,2), IN launch_date DATE, IN registration_deadline DATE, IN target_number_of_registration INTEGER, IN administrator_email TEXT, IN session_info SESSION_INFORMATION[])
AS $$
DECLARE
...
BEGIN
    ...
    administrator_id := (
        SELECT eid
        FROM Employees as e NATURAL JOIN Administrators as a
...
BEGIN
    ...
    INSERT INTO Course_packages(sales_start_date, sales_end_date, num_free_registrations, name, price)
...
BEGIN
    ...
    RETURN query (
        SELECT name AS package_name, num_free_registrations AS num_free_sessions, sales_end_date AS end_date, Course_packages.price AS price
        FROM Course_packages 
        WHERE sales_start_date <= cast(now() as DATE) 
        AND sales_end_date >= cast(now() as DATE) -- make sure the package is available on sale 
    );
...
BEGIN
    ...
    customer_id := (SELECT cust_id
                    ...
                    LIMIT 1); --find the customer id using the email

    credit_card := (SELECT card_number
                    FROM Owns_credit_cards
                    WHERE cust_id = customer_id
                    LIMIT 1); --one customer may have multiple credit cards, we choose the first one to use.

    card_expiry_date := (SELECT expiry_date
                         FROM Owns_credit_cards
                         WHERE card_number = credit_card);           

    num_free_sessions := (SELECT num_free_registrations 
                          FROM Course_packages
    ...
    IF (start_date <= now_date AND end_date >= now_date) --check whether the package is available for sale now 
    
    THEN
        IF (card_expiry_date < now_date) THEN --check whether the credit card is expired.
    ...
    ELSE
        RAISE EXCEPTION 'The course package is unavailable now!';
    END IF;
END;
$$ LANGUAGE plpgsql;
...
*/
/*Explanation and implementation:
1. I have to find the number from all the total number of sales. 
    Implementation: I used the pkg_info CTE to find and sort all the number of sales of all packages. 
2. I have to find all the packages which have the num of sales as top N among all the number of sales. 
   Implementation: I used the SELECT to find the corresponding information of the packages which satisfies the condition,
        and the subquery is used to select all the packages satisfies the condition and make sure all the package with top input_n number od sales can be selected.
*/
...
BEGIN
    ...
    WITH pkg_info AS (
        ...
        WHERE extract(year from sales_start_date) = extract(year from NOW())
        GROUP BY package_id
        ORDER BY num_sales DESC
    )

    SELECT package_id AS package_identifier, num_free_registrations AS num_sessions, price AS price_package, sales_start_date AS start_date, sales_end_date AS end_date, num_sales AS num_sold
    FROM Buys NATURAL RIGHT JOIN Course_packages NATURAL JOIN pkg_info
    WHERE extract(year from sales_start_date) = extract(year from NOW())
    AND num_sales = ANY( --num_sales equals to any number of top input_n num_sales and make sure any such package can be selected
...
BEGIN
    num_redemption := (
        ...
        FROM (Buys NATURAL LEFT JOIN Redeems) AS BR
...
BEGIN
    ...
    ELSEIF EXISTS (
        SELECT 1 
        FROM (Owns_credit_cards NATURAL JOIN Buys NATURAL JOIN Redeems NATURAL JOIN  Sessions) AS BROS
        WHERE BROS.cust_id = customer
        AND BROS.session_date - CAST(NOW() AS DATE)>= 7
    ) THEN
