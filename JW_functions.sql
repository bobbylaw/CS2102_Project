...
RETURNS TABLE (room_id INTEGER, room_capacity INTEGER, day DATE, free_hours INTEGER[])
AS $$
DECLARE
    hours_array INTEGER[24];
    temp_date DATE;
BEGIN


    RETURN query (
        temp_date := input_start_date;
        Loop
            day := temp_date
            WITH Room_info AS (
                SELECT rid  
                FROM Rooms NATURAL JOIN Sessions
                WHERE session_date = temp_date
                GROUP BY rid；
            ）
...
--Course_packages 中 package_id 需要是serial！！！
...
CREATE OR REPLACE FUNCTION get_my_course_packages(IN cust_id INTEGER)
RETURNS TABLE (Info JSON)
AS $$
DECLARE 
    info_redeemed_sessions[];
    package_name TEXT;
    num_free_sessions INTEGER;
    purchase_date DATE;
    price NUMERIC(12,2); 
    num_redeemed INTEGER;
    info_redeemed_sessions TEXT[]; --course name, session date, session start hour
BEGIN

    SELECT name INTO package_name, num_free_registrations into num_free_sessions, purchase_date INTO purchase_date, price INTO price, 
    FROM Course_packages NATURAL JOIN BUYS NATURAL JOIN redeems NATURAL JOIN Owns_credit_cards
    WHERE Owns_credit_cards.cust_id = cust_id;
